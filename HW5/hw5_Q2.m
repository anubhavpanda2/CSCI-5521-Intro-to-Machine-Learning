...
for i = 1:size(xGrid,1)
    ...
    for j = 1:N
        ...
        plotdata = plotdata + 1;
        plotdata = plotdata ^ 2;
        scorestotal = scorestotal + (a(j) * theclass(j) * plotdata);
    end
    scorestotal = scorestotal + b;
    scores(i) = scorestotal;
end
...
for i=1:size(data3,1)
    ...
    if sign(scorestotal + b) ~= theclass(i)
        train_err_rate = train_err_rate+1;
    end
        
end
train_err_rate = train_err_rate / size(data3,1);
disp(["Train error rate for data3 data", num2str(train_err_rate )]);
...
for i=1:size(train49,1)
    ...
    for j = 1:size(train49,1)
        ...
        plotdata = plotdata + 1;
        plotdata = plotdata ^ 2;
        scorestotal = scorestotal + (a49(j) * class49(j) * plotdata);
    end
    
    if sign(scorestotal + b49) ~= class49(i)
        train_err_rate = train_err_rate+1;
    end
...
for t = 1:size(test49,1)
    ...
    for i = 1:size(train49,1)
        ...
        plotdata = plotdata + 1;
        plotdata = plotdata ^ 2;
        scorestotal = scorestotal + (a49(i) * class49(i) * plotdata);
    end
    
    if sign(scorestotal + b49) ~= testclass49(t)
...
for i = 1:size(train79,1)
    ...
    for j = 1:size(train79,1)
        ...
        plotdata = plotdata + 1;
        plotdata = plotdata ^ 2;
        scorestotal = scorestotal + (a79(j) * class79(j) * plotdata);
    end
    
    if sign(scorestotal + b79) ~= class79(i)
...
for i = 1:size(test79,1)
    ...
    if sign(scorestotal + b79) ~= testclass79(i)
